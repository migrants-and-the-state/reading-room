<script>
	...
	const methods_glossary = [
		{
			'id': 'dnn',
			'term': 'Deep Neural Network (DNN)',
			'description': 'Analyzes and extracts important details from an image, such as shapes, colors, and patterns. Based on the analysis, the DNN then predicts how to classify the image.',
			'models': [{
				'name': 'DINOv2',
		...
		},
		{
			...
			'description': 'A type of artificial intelligence trained on vast amounts of images and text to understand and generate human-like language while also interpreting visual information.  They can analyze and describe images, answer questions about visual content, and generate text based on both visual and textual inputs. This allows them to perform tasks like image captioning, visual question answering, and multimodal reasoning.',
			'models': [{
				'name': 'MiniCPM-V-2_6',
		...
		},
		{
			...
			'description': 'A type of artificial intelligence where computers learn patterns from data instead of being explicitly programmed.'
		},
		{
			'id': 'ner',
			'term': 'Named Entity Recognition (NER)',
			'description': 'An NLP technique that identifies different types of entities such as locations, names, dates, and times by analyzing the surrounding context and linguistic patterns rather than relying solely on predefined rules like capitalization or keyword proximity.',
			'models': [{
		...
		},
		{
			...
			'description': 'A set of artificial intelligence techniques that enable computers to identify, understand, interpret, and generate human language.'
		},
		{
			'id': 'ocr',
			'term': 'Optical Character Recognition (OCR)',
			'description': 'A technology that detects and extracts text from images or scanned documents, converting it into machine-readable text.',
			'models': [{
		...
		},
		{
			'id': 'text',
			'term': 'Text Model',
			'description': 'An artificial intelligence system designed to process, generate, or analyze human language using machine learning techniques. In this case,  a sentence transformer that converts sentences into numerical embeddings, enabling tasks like semantic search, similarity comparison, and clustering.',
			'models': [{
...
<div class="max-w-[85ch]">
	...
	<p class="py-2">
		The NARA catalog metadata, compiled as a limited subset of the metadata created by INS / USCIS, often lacks entries corresponding to the field designating biological sex. But information on biological sex, along with other biometric data required by government forms, appears frequently across documents in A-Files. We used an open-source <Link size="lg" href="#llm">Large Language Model (LLM)</Link> to extract this data from the individual pages of A-Files, typically from forms where a biological sex designation (e.g.  Male / Female) is required. The model then aggregates the total number of identifications for biological sex and predicts the sex of the A-File holder. The LLM derived metadata is not 100% accurate, and the prototype allows you to compare NARA’s metadata for sex with the metadata extracted by the M/S trained LLM. 
	</p>
	<p class="py-2">
...
<div class="my-10 grid sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
	{#each methods_glossary as method}
		<Tile id="{method.id}">
			<dl class="w-full">
				<dt class="font-semibold text-base">{method.term}</dt>
				<dd class="py-2 leading-normal">
					{method.description}
					{#if method.models}
						<span class="text-xs block py-2 font-mono">
							M/S models used: {#each method.models as model, i}<OutboundLink size="sm" href={model.link}>{model.name}</OutboundLink>&nbsp;{/each}
						</span>
					{/if}
				</dd>
			</dl>
